...
@implementation JYHLSVProgressHUD

+ (JYHLSVProgressHUD*)sharedView {
    static dispatch_once_t once;
    static JYHLSVProgressHUD *sharedView;
    dispatch_once(&once, ^ {
...
+ (void)showWithMsg:(NSString *)msg{
    [JYHLSVProgressHUD setForegroundColor:[UIColor whiteColor]];
    [JYHLSVProgressHUD setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0.8]];
    [self sharedView].hudView.layer.cornerRadius = 5;
//    [self sharedView].hudView.alpha = 0.2;
    
    [self sharedView].hudView.layer.shadowColor = [UIColor blackColor].CGColor;//阴影的颜色
    [self sharedView].hudView.layer.shadowOpacity = 0.5f;//阴影的透明度
    [self sharedView].hudView.layer.shadowRadius = 5.f;//阴影的圆角
    [self sharedView].hudView.layer.shadowOffset = CGSizeMake(5,5);//阴影偏移量
    [self sharedView].hudView.layer.masksToBounds = NO;
    
    if (isIPhoneX) {
        
    }else{
     [[self sharedView] setOffsetFromCenter:UIOffsetMake(0, ScreenWidth *0.29866667)] ;
    }
